...
public class ForFromDownToExpression extends SingleExpression {
	private String variableName;
	private ValueExpression fromVe, toVe;

	public ForFromDownToExpression(String variableName, ValueExpression fromValue, ValueExpression toValue,
	...
	public ForFromDownToExpression(int label, String variableName, ValueExpression fromValue, ValueExpression toValue,
			Expression expression) {
	...
	private class CounterIncreaserExpression extends Expression {
		...
		public int write(Context ctx, Object additionalData) {
			...
			OperationsHelper.loadRegister(ctx, workingRegister,
					localVariable.getAssignedMemoryCells()[0].getCellAddress(BigInteger.ZERO));
			writer.write(OperationsHelper.genInstruction(Instructions.DEC_i, workingRegister));
